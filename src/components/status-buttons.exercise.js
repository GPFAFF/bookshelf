...
function StatusButtons({ user, book }) {
  ...
  const [update] = useMutation(({ id }) =>
    client(`list-items/${id}`, { data: { bookId: id }, token: user.token, method: 'PUT' }),
    {
      onSettled: () => queryCache.invalidateQueries('list-items')
    }
  )

  const [remove] = useMutation(({ bookId }) =>
    client(`list-items/${bookId}`, { data: { bookId }, token: user.token, method: 'DELETE' }),
    {
      onSettled: () => queryCache.invalidateQueries('list-items')
    }
  )
